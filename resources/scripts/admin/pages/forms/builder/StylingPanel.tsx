...
import { Check, MessageCircle, MessageSquare, PenLine, Smile, Star } from 'lucide-react';
import type {
	FormSettings,
	ProgressIndicator,
	TriggerIcon,
...
const NumbersPreview = ({ active }: { active: boolean }) => (
	<span className={cn('text-[10px] leading-none font-semibold tabular-nums', active ? 'text-primary' : 'text-foreground/40')}>
		1 / 3
	</span>
);

const BarPreview = ({ active }: { active: boolean }) => (
	<div className={cn('h-[4px] w-8 overflow-hidden rounded-sm', active ? 'bg-primary/20' : 'bg-foreground/10')}>
		<div className={cn('h-full w-2/5 rounded-sm', active ? 'bg-primary' : 'bg-foreground/30')} />
	</div>
);

const NonePreview = ({ active }: { active: boolean }) => (
	<span className={cn('text-sm leading-none font-medium', active ? 'text-primary/50' : 'text-foreground/30')}>—</span>
);

const PositionThumbnail = ({ position }: { position: WidgetPosition }) => {
	if (position === 'side-tab') {
		return (
			<svg viewBox="0 0 48 34" fill="none" className="h-8 w-full" aria-hidden="true">
				<rect x="1" y="1" width="36" height="32" rx="3" stroke="currentColor" strokeWidth="1.5" strokeOpacity="0.5" />
				<rect x="37" y="10" width="10" height="14" rx="2" fill="currentColor" fillOpacity="0.7" />
			</svg>
		);
	}
	if (position === 'bottom-right') {
		return (
			<svg viewBox="0 0 48 34" fill="none" className="h-8 w-full" aria-hidden="true">
				<rect x="1" y="1" width="46" height="32" rx="3" stroke="currentColor" strokeWidth="1.5" strokeOpacity="0.5" />
	...
	return (
		<svg viewBox="0 0 48 34" fill="none" className="h-8 w-full" aria-hidden="true">
			<rect x="1" y="1" width="46" height="32" rx="3" stroke="currentColor" strokeWidth="1.5" strokeOpacity="0.4" strokeDasharray="3 2" />
...
const PROGRESS_OPTIONS: {
	...
	{ value: 'numbers', label: __('Numbers', 'allfeedback'), Preview: NumbersPreview },
...
const ICON_OPTIONS: { value: TriggerIcon; label: string; Icon: React.ElementType }[] = [
	{ value: 'message', label: __('Message', 'allfeedback'), Icon: MessageSquare },
...
const optionBase = (active: boolean) =>
	cn(
		'group relative flex cursor-pointer flex-col items-center gap-2 rounded-xl border p-3 transition-all duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1',
		active
								...
								>
									<div className={cn(
										...
										isActive
											...
											: 'bg-muted/60 text-foreground/45 group-hover:bg-muted group-hover:text-foreground/65',
									)}>
