				...
				return (
					<div
						...
						style={{
							...
							color:      isDetractor ? '#dc2626' : isPassive ? '#d97706' : '#16a34a',
							background: isDetractor ? '#fef2f2' : isPassive ? '#fffbeb' : '#f0fdf4',
							...(i === 10 ? { background: color, borderColor: color, color: '#fff' } : {}),
						}}
					>
						{i}
					</div>
				...
				<div>
					...
					>
						<span
							...
							style={{ animation: 'allfb-wizard-dot-pulse 2s ease-in-out infinite' }}
						/>
					...
					<div
						...
						style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', padding: '18px 0' }}
					>
						{[
							{
							...
							},
							{
								...
								value: state.admin_email ? __('Email set', 'allfeedback') : __('Not set', 'allfeedback'),
								icon: state.admin_email ? <Check className="text-success size-3.5" strokeWidth={3} /> : null,
							},
						].map((stat, i) => (
							<div
								key={stat.label}
								className={i < 2 ? 'border-border border-r' : ''}
								style={{ padding: i === 0 ? '0 18px 0 0' : i === 2 ? '0 0 0 18px' : '0 18px' }}
							>
								<div
						...
						<div className="shrink-0 bg-[#dee1e6] select-none">
							<div className="flex items-end px-3 pt-2">
								...
								<div className="flex max-w-[172px] items-center gap-1.5 rounded-t-[7px] bg-white px-2.5 pt-[5px] pb-[6px]">
									...
									<span className="text-foreground/70 min-w-0 flex-1 truncate text-[8.5px]">yoursite.com</span>
									<X className="text-muted-foreground/35 size-2.5 shrink-0" />
								</div>
								<button type="button" className="ml-3 flex size-[18px] items-center justify-center rounded focus-visible:outline-none">
							...
							<div className="flex items-center gap-0.5 px-2 pt-2 pb-2">
								...
								<div className="mx-1.5 flex flex-1 items-center gap-1.5 rounded-full bg-white/95 px-3 py-[3px] shadow-[inset_0_0_0_1px_rgba(0,0,0,0.10)]">
									...
									<span className="text-foreground/70 flex-1 text-center text-[8.5px]">yoursite.com</span>
